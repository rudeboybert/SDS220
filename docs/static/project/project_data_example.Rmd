...
_Pipe your data frame into the `clean_names()` function from the `janitor` package. This will clean your variable names, making them easier to work with. Be sure to save your variable name cleaning by overwriting the original data frame by using the `<-` assignement operator._ 
...
_What is your identification (ID) variable (if you have one)? For more information on identification variables, read ModernDive Chapter 1.4.4_

Our identification variable is `school_name`; it allows us to uniquely identify each school in each row.
