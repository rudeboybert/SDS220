...
Consider the following 9 points saved in a data frame called `categ_regression_ex`:
...
* Boxplot recap
* In-class exercise, build in groups a as-of-yet unseen jitterplot with categorical x. We will follow up on Wednesday with same plot, but with bells and whistles added (baseline mean marked, offsets marked)
* 
...
* Show Spurious correlations
* Placehold for obvious (x, y, z) causal graph
* x: Gender -> y: Admissions. z: Competitiveness of getting in to department. Link to Simpson's paradox.
* Make a plot like 5.12 using `simple_regression_ex` and compute by hand all:
    + fitted values using equation
    + residauls
* minimizing residuals
    1. Compute for best-fitting line
    1. Compute for green dashed 5.13. Show this is bigger
