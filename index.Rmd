...
* "Summary functions" are many-to-one functions to compute "summary statistics," like `mean()` & `median`.
* Using summary functions to then `summarize()` rows
* Setting "group meta-data" of a data frame using `group_by()`, then `summarize()` rows
* One summary statistic of spread: standard deviation `sd()`, in particular when distribution of a numerical variable follows a normal AKA bell curve.
...
Third, the idea of "saving something in R" using the arrow `<-`. Ex: `x <- 5`. You do this so you can access it again later, either to
...
* Define the "pipe operator" `%>%`, pronounced *then*
