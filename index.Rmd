...
Recall `categ_regression_ex` data frame from from Lec11. Note that the explanatory variable $x$ = name is now categorical with 3 levels: Bert, Florence, Katie.

```{r, echo=FALSE, eval=TRUE}
categ_regression_ex
```

Let's plot them by hand. 

Now let's obtain the *regression table*. Recall this is done in two steps:
```{r, eval = TRUE}
...
What do the three "estimate" terms (`intercept` = 10, `nameFlorence` = 2, and `nameKatie` = -1) mean?


<!--
* How to bridge gap between output of regression table & wacked out indicator function notation in Chapter 
    + Show regression table using Bert, Florence, Katie data
    + Then make separate table of name on rows, and nameFlorence and nameKatie in columns, populate with 0's & 1's
    + Write out equation of "line" in terms of regression table output `value = intercept + nameFlorence + nameKatie`.
    + Then write out equation of line via math formula y = f(x) with indicator functions.
* Emphasize that
    + fitted values are group means. Do it for Bert, Florence, Katie, 
    + Compute a few residuals
* To ModernDive -> Chapter 5.2.2 -> a visualization of what the terms in the regression table in Table 5.8 actually mean:
    ...
    + If Katie has bandwidth, add to [moderndive_book](https://github.com/moderndive/ModernDive_book/) repo -> `bert-dev` branch -> Open `05-regression.Rmd` and edit, commit, and push this file only. The book builds and deploys automatically using Netlify
-->
...
<!--
1. When you want to compare distribution of a numerical variable (y-axis) split by an x categorical variable
1. Remember, solid line in middle is median, not mean. Both are measures of center, one is more sensitive to outliers.

Ex:
Say a company has 5 employees with hourly salary: 10, 10, 10, 10, 1000. Mean hourly salary is $208. Why might this not be appropriate?
-->

## Chalk talk

Consider the following 9 points saved in a data frame called `categ_regression_ex`. Note that the explanatory variable $x$ = name is now categorical with 3 levels: Bert, Florence, Katie.
...
In-class exercise in groups: Plot a scatterplot with y = value (numerical) and x = name (categorical). Note this is slightly different that the scatterplots we've seen until now where the x variable was always numerical

Add code to actually create `categ_regression_ex` data frame


<!--
Not sure if we should do this today, or next lecture?!?

After students have done this, on scatterplot above mark:
1. (baseline) mean for Bert with horizontal line.
1. mean for Florence and Katie with horizontal line.
