...
* By tomorrow:
    + I will post solutions to Practice Midterm III.
    + Precise instructions for Midterm III.
        1. For example: you will need a calculator.
        1. Topics are Chapter 8-11.
* I will be on Slack during reading week, so please post questions in `#questions` channel.
...
* Exit survey [posted](term_project.html#to-submit). 2.5% of your final project grade is based on merely completing this exit survey.
* In particular questions relating to use of Slack, as at the Symposium on Data Science and Statistics (SDSS) I'll be presenting ["Using Slack for Communication and Collaboration in the Classroom"](https://ww2.amstat.org/meetings/sdss/2019/onlineprogram/AbstractDetails.cfm?AbstractID=305068){target="_blank"}  
</br>
<center>
...
* Office hours during reading/exam week by appointment. If someone has already booked a time you want to attend, please come anyways; the more the merrier! Book [here](https://calendar.google.com/calendar/selfsched?sstoken=UVBidEVzZlJ3a2dYfGRlZmF1bHR8NmExNDk4OGM5ZWZiYWNmOWYxYWRkZDM0NmU3ZGRmMzE){target="_blank"}
* Practice Midterm III to be posted under [midterms](midterms.html#midtermIII) by Wednesday.
...
* I will be absent on Friday. Guest lecturer: [Prof. Ben Capistrant](https://ssw.smith.edu/academics/faculty_detail/benjamin-capistrant){target="_blank"} from the Smith School of Social Work and current SDS/MTH 291 Multiple Regression instructor. 
...
* Interpreting hypothesis testing outcomes
...
* Term project resubmission instructions posted under [Term Project](term_project.html#resubmission).
...
* The `r emo::ji("confused")``r emo::ji("face_with_head_bandage")``r emo::ji("dizzy_face")``r emo::ji("exploding_head")``r emo::ji("scream")` statistical definitions, terminology, and notation for hypothesis testing.
* Remember that at the root of all hypothesis testing, just like with confidence intervals, is  **sampling**!  
<center>
![](static/images/bowl.jpg){ width=350px }
</center>
...
For those of you who are Game of Thrones fans and those of you who are potentially interested in Machine Learning, check out this [tweet](https://twitter.com/kierisi/status/1118144762173886466){target="_blank"}. Season 8, Episode 1 spoiler alert!
...
* Discussion on census bureau [checkboxes for "race"](https://www.kpbs.org/news/2013/jan/09/census-asks-latino-ethnicity-or-race/){target="_blank"} and the [politics](https://www.amacad.org/publication/racial-classification-america){target="_blank"} behind their determination. 
...
If you care to share feedback relating to the use of this example, please fill out this [Google Form](https://docs.google.com/forms/d/e/1FAIpQLSf0apU3UZ3RH0BhWYMB9io9XbcYXb-awxWpYY204JlHXvPK1g/viewform){target="_blank"}. You have the option to remain anonymous and all responses will remain confidential. 

<blockquote class="twitter-tweet" data-lang="en"><p lang="en" dir="ltr">Thanks to <a href="https://twitter.com/AmstatNews?ref_src=twsrc%5Etfw">@AmstatNews</a> for recognizing the importance of the gender spectrum, especially wrt the statistics classroom and study design.  I&#39;m really proud of this article with Jack Miller.  Also thanks to <a href="https://twitter.com/Miles_Ott?ref_src=twsrc%5Etfw">@Miles_Ott</a> !!<br> <a href="https://t.co/jlxMoVqWir">https://t.co/jlxMoVqWir</a></p>&mdash; Jo Hardin (@jo_hardin47) <a href="https://twitter.com/jo_hardin47/status/1101613988868055041?ref_src=twsrc%5Etfw">March 1, 2019</a></blockquote>
...
* Watch [slideshow](static/Lec32.pdf){target="_blank"}
...
View(promotions)
```
* Fill in [worksheet](worksheet/Lec32.pdf){target="_blank"}
* **Results**:  
![](static/promotions.jpg){ width=100% }
...
* Midterm II review. In particular practice midterm posted on [Midterms](midterms.html#midtermII) page.
* Project feedback: Sign up for a feedback session next week where all group members and myself will record a screencast in my office. **Please be mindful of how much coordination it takes for me to schedule 17 feedback sessions and read all these instructions carefully first**:
    1. Jump to the week of April 14-20 (next week) on my [Google Appointments Calendar](https://calendar.google.com/calendar/selfsched?sstoken=UVBidEVzZlJ3a2dYfGRlZmF1bHR8NmExNDk4OGM5ZWZiYWNmOWYxYWRkZDM0NmU3ZGRmMzE){target="_blank"} and identify which "220 Project Feedback Only" 15 minute time slots work for you.
    1. Then as a group, coordinate on a "220 Project Feedback Only" 15 minute time slot **when all group members can attend.** 
    1. Then only the group leader will book the 15 minute appointment. Under "Description" include both 1) your group name and 2) the names of all group members.
    1. If none of the listed times work for you all, contact me in the Slack Direct Message that includes me and Jenny and all group members.
...
![](static/images/infer.png){ width=500px }
...
* What determines the width of net?
    ...
    1. The original sample size n. As n goes up, the CI gets narrow i.e. you have more precise results.
...
ggplot(bootstrap_distn , aes(x = sample_mean)) +
  geom_histogram(binwidth = 2.5) +
  labs(x = "sample mean year", 
       title = "Bootstrap resampling distribution + 95% confidence interval") +
...
* Recap of Lec23:
    + Recall your "tactile" resampling results in this [Google Sheet](https://docs.google.com/spreadsheets/d/1y3kOsU_wDrDd5eiJbEtLeHT9L5SvpZb_TrzwFBsouk0/edit?usp=sharing){target="_blank"}
    + Let's load each of your resampled sample means and plot a histogram.
* Today:
    + **New inference scenario** Number 2 in moderndive [Table 8.6](https://moderndive.netlify.com/8-sampling.html#sampling-conclusion-table){target="_blank"}. Unknown value is no longer population proportion $p$ but population mean $\mu$
...
* What is the [`tidyverse`](https://www.tidyverse.org/packages/){target="_blank"} package?  
<center>
...
```{r, echo = FALSE}
if(!file.exists(here("static", "resample_data.RData"))){
  library(tidyverse)
  library(googledrive)
  library(googlesheets)
  drive_find(n_max = 25)
  
  resamples <- "1y3kOsU_wDrDd5eiJbEtLeHT9L5SvpZb_TrzwFBsouk0" %>%
    gs_key() %>%
    gs_read(ws = 1)  %>%
    select(-`Resampled penny #`) 
  
  # Get names
  student_names <- names(resamples)
  
  # Write tibble
  resamples <- resamples %>%
    apply(2, mean) %>%
    tibble(name = student_names, sample_mean = .)
  
  # Save
  write_csv(resamples, here("static", "resample_means.csv"))
}
```

Resampling tactile exercise. Open this [Google Sheet](https://docs.google.com/spreadsheets/d/1y3kOsU_wDrDd5eiJbEtLeHT9L5SvpZb_TrzwFBsouk0/edit?usp=sharing){target="_blank"}. Resulting 35 sample means based on a resample of size 50 are here:

```{r, eval=TRUE, echo=FALSE}
resamples <- read_csv(here("static", "resample_means.csv"))
ggplot(resamples, aes(x = sample_mean)) +
  geom_histogram(binwidth = 2.5) +
  labs(x = "sample mean year", title = "Bootstrap resampling distribution")
```
...
* Discussion on [sampling distribution handout](static/sampling_distribution_handout.pdf){target="_blank"}.
