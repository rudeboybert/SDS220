...
example <- tibble(
  name = c("Bert", "Bert", "Bert", "Katie", "Katie", "Katie", "Florence", "Florence", "Florence"),
  value = c(9, 10, 11, 11, 12, 13, 8, 9, 10)
)

ggplot(example, aes(x=name, y= value)) +
  geom_point()
...
* Add to ModernDive a jitterplot of x = continent, y = lifeExp, mark baseline, mark means, show offsets of marked means vs baseline. Insert somewhere logical. Right before "Let’s generalize this idea a bit". 
* Show regression table
* Then make separate table of name on rows, and nameFlorence and nameKatie in columns, populate with 0 & 1
* Do it first expressed in R code `value = intercept + nameFlorence + nameKatie`. Then do in math formula y = f(x) with indicator
* Emphasize that
    + fitted values are group means. Do it for Bert, but also for Jenny
...
* Boxplot recap
* In-class exercise, build in groups a as-of-yet unseen jitterplot with categorical x. 
* 
